BeforeAll {
    ...
    $script:TestCatalogName = "test_monitor_catalog_$(Get-Random)"
    $script:TestSchemaName = "test_monitor_schema_$(Get-Random)"
    $script:TestTableName = "test_monitor_table_$(Get-Random)"
    $script:TestFullTableName = ""
}

Describe "Databricks Lakehouse Monitor Bicep Module Tests" {
    BeforeAll {
        # Create test catalog, schema, and table for monitor
        Write-Host "Creating test catalog: $($script:TestCatalogName)"
        $catalogConfig = @{
            name = $script:TestCatalogName
            comment = "Test catalog for lakehouse monitor tests"
            isolation_mode = "OPEN"
        ...
        try {
            ...
            $schemaConfig = @{
                ...
                comment = "Test schema for lakehouse monitor tests"
            } | ConvertTo-Json -Depth 10
    ...
    Context "Positive Path Tests" {
        It "Should create a basic lakehouse monitor" {
            if (-not $script:TestFullTableName) {
                Set-ItResult -Skipped -Because "Test table creation failed"
                return
            }
            
            $templateFile = "$PSScriptRoot/../modules/lakehouse-monitor.bicep"
            $parameters = @{
                TableName = $script:TestFullTableName
                DatabricksToken = $script:TestToken
                WorkspaceUrl = $script:TestWorkspaceUrl
                AssetsDir = "/Shared/lakehouse-monitors/test-monitor-$(Get-Random)"
            ...
            $script:CreatedMonitorTableName = $deployment.Outputs.TableName.Value
        }
        
        It "Should create a lakehouse monitor with custom metrics" {
            if (-not $script:TestFullTableName) {
                Set-ItResult -Skipped -Because "Test table creation failed"
                return
            }
            ...
            $parameters = @{
                ...
                DatabricksToken = $script:TestToken
                WorkspaceUrl = $script:TestWorkspaceUrl
                AssetsDir = "/Shared/lakehouse-monitors/test-custom-monitor-$(Get-Random)"
                OutputSchemaName = "$($script:TestCatalogName).$($script:TestSchemaName)"
                CustomMetrics = @(
                    @{
                        type = "aggregate"
            ...
            $script:CreatedCustomMonitorTableName = $deployment.Outputs.TableName.Value
        }
    }
    ...
    AfterAll {
        # Cleanup monitors
        if ($script:CreatedMonitorTableName) {
            Write-Host "Cleaning up lakehouse monitor: $($script:CreatedMonitorTableName)"
            try {
                $secureToken = ConvertTo-SecureString $script:TestToken -AsPlainText -Force
                & "$PSScriptRoot/../helpers/Invoke-DatabricksApi.ps1" `
                    -Method "DELETE" `
                    -UrlPath "/api/2.1/unity-catalog/monitors/$($script:CreatedMonitorTableName)" `
                    -DatabricksToken $secureToken `
                    -WorkspaceUrl $script:TestWorkspaceUrl
            }
            catch {
                Write-Warning "Failed to cleanup lakehouse monitor $($script:CreatedMonitorTableName): $($_.Exception.Message)"
            }
        }
        
        if ($script:CreatedCustomMonitorTableName) {
            Write-Host "Cleaning up custom lakehouse monitor: $($script:CreatedCustomMonitorTableName)"
            try {
                $secureToken = ConvertTo-SecureString $script:TestToken -AsPlainText -Force
                & "$PSScriptRoot/../helpers/Invoke-DatabricksApi.ps1" `
                    -Method "DELETE" `
                    -UrlPath "/api/2.1/unity-catalog/monitors/$($script:CreatedCustomMonitorTableName)" `
                    -DatabricksToken $secureToken `
                    -WorkspaceUrl $script:TestWorkspaceUrl
            }
            catch {
                Write-Warning "Failed to cleanup custom lakehouse monitor $($script:CreatedCustomMonitorTableName): $($_.Exception.Message)"
            }
        }
